...
- **Bulk Fixing:** Implemented in `src/components/ReviewMode.tsx`. The "Smart Fix All" button applies safe formatting anomaly fixes (Name, Phone, Address) automatically across the entire dataset shown in the Review Mode without requiring manual review per record.
