                            ...
                            <input
                                ...
                                onChange={async (e) => {
                                    ...
                                    if (newZip.length === 5) {
                                        ...
                                        if (enrichedAsync) {
                                            setTargetAddress(prev => {
                                                if (prev.zip !== newZip) return prev; // Avoid overriding if user kept typing
                                                const updated = { ...prev };
                                                const localEnriched = enrichZipCode(newZip);
                                                if (!updated.city || updated.city === localEnriched?.city) {
                                                    updated.city = enrichedAsync.city;
                                                }
                                                if (!updated.state || updated.state === localEnriched?.state) {
