...
export const AutomationsReport: React.FC<AutomationsReportProps> = ({ students, graderOptions }) => {
    ...
    const firstTimeGivers = useMemo(() => {
        return getFirstTimeGivers(students, today, 7).filter(g => !dismissedFirstTimeGivers.has(g.person.id));
    }, [students, today, dismissedFirstTimeGivers]);

    const totalActions = newBabies.length + birthdays.length + promotions.length + sendOffs.length + expiringChecks.length + expiredChecks.length + firstTimeGivers.length;
    ...
    const handleApprove = (id: string, action: string) => {
        ...
        if (action === 'Draft Thank You Note') handleDismissFirstTimeGiver(id);
    };
            ...
            <div className="automation-lanes">
                ...
                <section className="automation-lane">
                    ...
                    {firstTimeGivers.length === 0 ? (
                        ...
                        <ul className="action-list">
                            {firstTimeGivers.map(({ person, daysSinceDonation }) => (
                                <li key={person.id} className="action-item">
                                    <div className="action-details">
                                        <strong>{person.name}</strong>
                                        <div className="meta">First gift {daysSinceDonation === 0 ? 'today' : `${daysSinceDonation} days ago`}</div>
                                    </div>
                                    <div className="action-buttons">
                                        <button className="btn-approve" onClick={() => handleApprove(person.id, 'Draft Thank You Note')}>Draft Thank You Note</button>
