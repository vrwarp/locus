...
import { transformPerson, updatePerson, fetchAllPeople, archivePerson, fetchCheckInCount, fetchDonationTotal } from './utils/pco'
...
function App() {
  ...
  const handleAnalyzeGhosts = async (ghostsToAnalyze: Student[]) => {
      ...
      // We need to update the query cache with the new checkInCount and donationTotal
      ...
      const updates = await Promise.all(ghostsToAnalyze.map(async (ghost) => {
          const [checkInCount, donationTotal] = await Promise.all([
              fetchCheckInCount(ghost.id, auth),
              fetchDonationTotal(ghost.id, auth)
          ]);
          return { id: ghost.id, checkInCount, donationTotal };
      }));
